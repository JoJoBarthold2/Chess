...
def main(args):
    ...
    if args.output_file:
        _create_output_file(args.output_file)

        raw_output_file = args.output_file.replace('.txt', '') + f'{args.num_games}_raw_output.txt'
        _create_output_file(raw_output_file)
    ...
    while running:
                    ...
                    if args.verbose:
                        ...
                        if args.output_file:
                            ...
                            with open(raw_output_file, 'a') as f:
                                f.write(str(ai_move.getRawMove()) + ';')
                ...
                if not game_state.illegal_move_done:
                    ...
                    if args.verbose:
                        ...
                        if args.output_file:
                            ...
                            with open(args.output_file, 'a') as f:
                                print(f"{str(ai_move)}\n")
                                print(f"{str(ai_move.getRawMove())}\n")
                                f.write(s)
                            with open(raw_output_file, 'a') as f:
                                f.write(str(ai_move.getRawMove()) + ';')
                    # if ai_move is None:
        ...
        if game_over == True:
            ...
            if num_games > 0:
                ...
                raw_output_file = args.output_file.replace('.txt', '') + f'{num_games}_raw_output.txt'
                _create_output_file(raw_output_file)
