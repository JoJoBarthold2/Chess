...
def main(args):
    ...
    GameTable = {"Draw by 50 move rule": 0, "Draw by threefold position repetition": 0, "Black wins by checkmate": 0,
                 ...
                 "Black wins by illegal move": 0, 'Draw by stalemate': 0}

    start_time = -1
    waiting = False  # to pause replay
    ...
    if args.agent1 == 'MrReplay':
        if args.game_log_file:
            with open(args.game_log_file, 'r') as f:
                ll = f.readlines()
            move_log = [l.replace('\n', '') for l in ll]
            if len(move_log) == 1:
                move_log = move_log[0].split(';')
            if move_log[-1] == '':
                move_log = move_log[:-1]
            move_log = [make_tuple(l) for l in move_log]
        elif args.game_log_str:
            move_log = args.game_log_str.split(';')
            if move_log[-1] == '':
                move_log = move_log[:-1]
            move_log = [make_tuple(l) for l in move_log]
        else:
    ...
    while running:
        ...
        for e in py.event.get():
            ...
            # key operations
                ...
                #        ai_thinking = False
                #    move_undone = False
                if e.key == py.K_SPACE:  # pauce when spacebar is pressed.
                    waiting = not waiting
        ...
        if not game_over and not human_turn and not move_undone:
            ...
            if waiting:
                start_time = time.time() + clock_counter - args.time_control

            if not waiting and (time.time() - start_time > args.time_control or not move_finder_process.is_alive()):
        ...
        if game_state.checkMate or game_state.staleMate or halfmoveClock == 100 or game_state.threefold or game_state.draw or game_state.illegal_move_done:
            game_over = True
...
if __name__ == "__main__":
    ...
    parser.add_argument('--game_log_file', type=str, default=None,
                        help="File with raw output of the moves from a previous game.")
    parser.add_argument('--game_log_str', type=str, default=None,
                        help="Raw output of the moves from a previous game as a string, delimited by ';'.")
