...
o First, you need to install the python packages pygame and tkinter by executing the following command in a terminal:
...
o The game gui can be started by running the script 'ChessMain.py' in the following way:

  § python3 ChessMain.py --agent1 [*] --agent2 [*] --verbose --time_control [how many seconds either player gets per move] --use_gui
  
  For [*] you can put either 'MrRandom', 'MrNovice', 'Agent1', 'Agent2', 'Human', or a path to your agent file. 'MrRandom' will play
  completely random moves (valid moves). 'MrNovice' will play moves according to the output of a minimax algorithm.
  'Agent1' will use the class Agent from 'student_agents/template.py' while 'Agent2' will use the class Agent from
  'student_agents/template2.py'. By entering 'Human' you act as the agent and can
  play yourself (if the gui is activated).
  WINDOWS USERS: Here you need to use one of the fixed arguments, it does not work to use the file path.
...
o FOR THE SUBMISSION you must rename the file studentagent.py to the last name of both team members 
  (e.g. frank_vetter.py} for Frank and Vetter). Keep the class name as 'Agent'. Your agent's
...
o If you find any inefficiency in the code, e.g in ChessMain.py, ChessEngine.py,
  ...
  martin.messmer@uni-tuebingen.de until 22.12.23.
